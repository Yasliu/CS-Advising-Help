function drawline(container, fromId, toId) {
    // goes from which point to which point and resides where(container)
    ...
    const fromBox = from.getBoundingClientRect(); // getBoundingClientRect() is used to obtain the position
    const toBox = to.getBoundingClientRect();
    const containerBox = container.getBoundingClientRect();

    // Making the line itself
    const line = document.createElementNS("http://www.w3.org/2000/svg", "line")

    // Dealing with positioning of the lines
    ...
    // Setting up the stroke width, color etc.
...
function renderFlowchart(containerId, layout, connections) {
    ...
    for (const [courseId, pos] of Object.entries(layout)) {
        ...
        el.style.left = `${(pos.x / max_X) * 100}%`;
        el.style.top = `${(pos.y / max_Y) *100}%`;
...
// Fetching info from Courseinfo.html
...
fetch("courseInfo.html")
    ...
    .then(html => {
        ...
        document.body.appendChild(courseInfoStore);
    })
    .catch(err => {
...
const electiveLayout = {
  ...
  // ensuring copy = original for info
